...
def plot_marker_genes_group(adata, group, marker_gene_list, leiden='leiden', layer=None, use_raw=True, 
                            save_fig=True, output_path="../output/" + tissue + "_UMAP_violin_plot_for_"):
    """
...
def plot_marker_genes(adata, marker_gene_list, leiden = 'leiden', layer = None, use_raw = True, save_fig = True,
                      output_path = "../output/" + tissue + "_UMAP_violin_plot_for_"):
...
def proportion_plot(adata, group, group1, group2, leiden_col, sample_id_col, error_bars = True, 
                    save_fig = True, output_name = "", output_path = '../output/' + tissue + '_proportions_labeled_'):
    """
    ...
    output_path : str, optional
        Path where the figure will be saved if `save_fig` is True. Default is '../output/' + tissue + '_proportions_labeled_'.
...
def save_dfs(stat_res, celltype, group, group1, group2, pval_lim = 0.05, lFC_lim = 1, more_info = ''):
    ...
    de.to_csv(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_deseq_all_genes.txt', sep = '\t')
    deg.to_csv(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_deseq_degs.txt', sep = '\t')
    top_genes.to_csv(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_deseq_top_genes.txt', sep = '\t')
    bottom_genes.to_csv(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_deseq_bottom_genes.txt', sep = '\t')
    
    print(f'files saved to: ../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison....')
    return(de)
...
def save_plot(fig, save, celltype, group, group1, group2, more_info = ''):
    if save is not None:
        if fig is not None:
            fig.savefig(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_Volcano_Plot.png', 
                        bbox_inches='tight')
        else:
...
def plot_volcano_df(data, celltype, group, group1, group2, more_info = '', 
    ...
    ax.set_title(f'DEGs in {group2} vs {group1}' + str.replace(more_info, '_', ' ') + f' {tissue} ' + str.title(str.replace(celltype, '_', ' ')),
...
def save_plot_html(fig, save, celltype, group, group1, group2, more_info = ''):
    if save is not None:
        if fig is not None:
            fig.write_html(f'../output/DEGs/{tissue}_{celltype}_{group2}{more_info}_vs_{group1}{more_info}_{group}_comparison_Volcano_Plot_Interactive.html')
        else:
            raise ValueError("fig is None, cannot save figure.")
...
def plot_volcano_df_html(data, celltype, group, group1, group2, more_info = '', 
    ...
    fig.update_layout(
        ...
        title=f'<b>DEGs in {group2} vs {group1}' + str.replace(more_info, '_', ' ') + f' {tissue} ' + str.title(str.replace(celltype, '_', ' ')) + '</b>',
