...
bright_palette = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#17becf"]
### 28 color palette
...
def plot_marker_genes_group(adata, group, marker_gene_list, leiden='leiden', layer=None, use_raw=True, 
                            save_fig=True, output_path=f"../output/{save_name}_UMAP_violin_plot_for_"):
    """
    ...
    sc.pl.umap(adata, ax=ax1, color=leiden, size=30, add_outline=True, legend_loc='on data',
    ...
    sc.pl.umap(adata, ax=ax2, color=[score_name], size=30, add_outline=True, show=False,
...
def plot_marker_genes(adata, marker_gene_list, leiden = 'leiden', layer = None, use_raw = True, save_fig = True,
                      output_path = f"../output/{save_name}_UMAP_violin_plot_for_"):
...
#                     save_fig = True, output_name = "", output_path = f'../output/{save_name}_proportions_labeled_'):
#     """
...
### Builds a dataframe with the number of cells of each cell type for each sample, 
# as well as the total number of cells for each sample 
# and the proportion of each cell type for each sample
def build_prop_df(adata, celltype_col):
    df_len = len(adata.obs.sample_id.unique()) * len(adata.obs[celltype_col].unique())
    df = pd.DataFrame(index=range(df_len),
                    columns = ['sample_id', 'sample_type', 'tissue', 'celltype', 'celltype_number', 'total_number'])
    ...
    for sample_id in adata.obs.sample_id.unique():
        ...
        for celltype in adata.obs[celltype_col].unique():
                ...
                df.sample_type.iloc[i] = df_sample.sample_type.iloc[0]
                df.celltype.iloc[i] = celltype
                df.tissue.iloc[i] = df_sample.tissue.iloc[0]
                df.total_number.iloc[i] = len(df_sample)
                df.celltype_number.iloc[i] = len(df_sample[df_sample[celltype_col] == celltype])
                i+=1
...
### Calculate Welch's t-test for proportions and builds a dataframe with the results
def calc_prop_stat(df, comparisons):
    results = []

    # Group once instead of repeatedly filtering
    grouped = df.groupby(["celltype", "sample_type"])["proportion"]

    for sample_type_1, sample_type_2 in comparisons:

        for celltype in df["celltype"].unique():

            group1 = grouped.get_group((celltype, sample_type_1))
            group2 = grouped.get_group((celltype, sample_type_2))
...
### Plotting function
def proportion_plot_across_samples(df, custom_labels, save_name):
    plt.figure(figsize = (16,6))
    plt.style.use("seaborn-v0_8-white")

    group_order = ['Thoracic_Aorta_Mesothelium','Mesentary_Mesothelium', 'Kidney_Mesothelium']

    cell_type_order = df.groupby('celltype').mean(numeric_only=True).sort_values(by = "proportion", ascending=False).index.tolist()

    ax = sns.barplot(data = df, x = 'celltype', y = 'proportion', capsize = 0.06, edgecolor = '0.2', lw = 2.5, 
                    errorbar='se', err_kws={'color': '0.2', 'linewidth': 2},
                    hue = 'sample_type', order = cell_type_order, hue_order = group_order, palette = 'Purples_r',
                    )
    ...
    plt.legend(handles[:], labels[:], loc = 'upper right', # bbox_to_anchor = (1, 0), 
            ncol = 1, frameon = False, fontsize = 16, labelcolor = '0.2')#, prop = {'weight': 'bold'})
    ...
    plt.yticks(size = 14, color = '0.2') #weight = 'bold',
    ...
    # plt.savefig(f'../output/{save_name}.png', bbox_inches = 'tight', dpi = 600, facecolor = ax.get_facecolor())
    #plt.show()
